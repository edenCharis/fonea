                  ...
                  <tbody>
                   ...
                   @endforeach
                    ...
                    <td> 
                             ...
                             <a href="{{ route("autre.detailsfq", ["id" => $d->id])}}" class="btn btn-sm btn-info"> <i class="fa fa-info"></i> </a>
                             <form action="{{ route('formation_qualifiante.destroy', $d->id) }}" method="POST" onsubmit="return confirm('Etes vous sûr de vouloir supprimer cette ligne ?');">
                @csrf
                @method('DELETE')
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                Action de formation qualifiante  </h5>
               </div>
            <form id="trimestreForm" method="POST" action="{{ route('formation_qualifiante.store') }}">
                @csrf
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                Détailler une action de formation  </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="add1" method="POST" action="{{ route('detailsFQ.store') }}">
                @csrf
                <div class="modal-body">
                <div class="mb-3">
                        <label for="formation_qual_id" class="form-label">Selectionnez l'action de formation </label>
                        <select id="formation_qual_id" name="formation_qual_id" class="form-select" required>
                            <option value="" disabled selected></option>
                            @foreach($data as $s)
...
<div class="modal fade" id="add2" tabindex="-1" aria-labelledby="add2" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="add2" method="POST" action="{{ route('realisationFQ.store') }}">
                @csrf
                <div class="modal-body">
                <div class="mb-3">
                <label for="formation_qual_id" class="form-label">Selectionnez l'action de formation </label>
